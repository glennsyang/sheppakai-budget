...
describe('formMessageFromUrl', () => {
	it('returns undefined when no message is present', () => {
		expect(formMessageFromUrl(at('/sign-in'))).toBeUndefined();
	});

	it('reads an explicit success message', () => {
		expect(
			formMessageFromUrl(at('/sign-in?message=Password%20reset%20successful&messageType=success'))
		).toEqual({ type: 'success', text: 'Password reset successful' });
	});

	it('reads an explicit error message', () => {
		expect(
			formMessageFromUrl(at('/sign-in?message=Invalid%20verification%20link&messageType=error'))
	...
	it('falls back to error when the type is missing or unrecognised', () => {
		...
		expect(formMessageFromUrl(at('/sign-in?message=Your%20account%20is%20verified'))?.type).toBe(
			'error'
		);
		expect(formMessageFromUrl(at('/sign-in?message=Nice%20try&messageType=SUCCESS'))?.type).toBe(
			'error'
		);
	});
});
