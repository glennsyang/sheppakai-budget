	...
	<Card.Content class="pt-0">
		{#if goals.length === 0}
			<div class="flex flex-col items-center gap-2 py-6 text-center">
				<TargetIcon class="size-8 text-muted-foreground/40" />
				<p class="text-sm text-muted-foreground">No active savings goals</p>
				<a href="/savings/goals" class="text-xs text-primary hover:underline">Create a goal →</a>
			</div>
		{:else}
