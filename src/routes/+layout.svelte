<script lang="ts">
	...
	import IncomeDialog from '$lib/components/IncomeDialog.svelte';
...
{:else}
		...
		>
			<div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
				<div class="flex items-center justify-between">
					...
					<div class="flex items-center space-x-4">
						{#if data.user}
							<span class="text-sm text-muted-foreground">Welcome, {data.user.email}</span>

							<!-- Dropdown Menu for Actions -->
							<DropdownMenu>
								<DropdownMenuTrigger>
									<Button variant="outline" size="sm" class="gap-2">
										Menu<MoreVerticalIcon class="h-4 w-4" />
	...
	<!-- Expense Dialog -->
	<ExpenseDialog
		bind:open={expenseDialogOpen}
		onOpenChange={(open) => (expenseDialogOpen = open)}
		{onExpenseAdded}
	/>

	<!-- Income Dialog -->
	<IncomeDialog
		bind:open={incomeDialogOpen}
		onOpenChange={(open) => (incomeDialogOpen = open)}
		{onIncomeAdded}
	/>
{/if}
